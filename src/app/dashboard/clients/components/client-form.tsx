...
export function ClientForm({ client, onSaved }: ClientFormProps) {
  ...
  return (
    <div className="max-w-2xl space-y-6">
            ...
            {[{ value: 'male', label: 'Male' }, { value: 'female', label: 'Female' }].map((g) => (
              <button
                ...
                className={`flex-1 py-2.5 rounded-lg border text-sm font-medium transition-all ${
                  gender === g.value ? 'border-gold bg-gold/10' : 'border-border hover:border-gold/50'
                }`}
              >
                {g.label}
              </button>
      ...
      {/* Notes */}
      <section className="space-y-4">
        <h3 className="text-sm font-medium text-muted-foreground uppercase tracking-wider">Preferences & Notes</h3>
