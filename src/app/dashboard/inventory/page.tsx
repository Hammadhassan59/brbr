    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        {[
          ...
          <Card key={c.label} className={c.urgent ? 'border-amber-500/25 bg-amber-500/5' : ''}>
            ...
            {loading ? <div className="h-20 bg-muted rounded animate-pulse" /> : lowStock.length === 0 ? (
              ...
              <div className="space-y-2">
                {lowStock.slice(0, 5).map((p) => (
                  <div key={p.id} className="flex items-center justify-between p-2 bg-red-500/10 rounded-lg border border-red-500/15">
                    <div>
      ...
      <div className="flex flex-wrap gap-2">
        <Link href="/dashboard/inventory/products"><Button variant="outline" size="sm">Products</Button></Link>
        <Link href="/dashboard/inventory/suppliers"><Button variant="outline" size="sm">Suppliers</Button></Link>
        <Link href="/dashboard/inventory/orders"><Button variant="outline" size="sm">Purchase Orders</Button></Link>
      </div>
    </div>
  );
}
