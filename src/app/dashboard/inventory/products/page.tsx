  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-wrap items-center gap-2">
        ...
        <Button onClick={() => openForm()} className="bg-gold text-black border border-gold" size="sm"><Plus className="w-4 h-4 mr-1" /> Add Product</Button>
      </div>
      ...
      <Tabs value={tab} onValueChange={(v) => setTab(v as Tab)}>
        <TabsList className="h-auto gap-1 flex-wrap">
          ...
          <TabsTrigger value="backbar" className="text-xs">Backbar</TabsTrigger>
          <TabsTrigger value="retail" className="text-xs">Retail</TabsTrigger>
          <TabsTrigger value="low" className="text-xs">Low Stock ({products.filter((p) => p.current_stock <= p.low_stock_threshold).length})</TabsTrigger>
        </TabsList>
      </Tabs>
      ...
      <div className="flex flex-wrap gap-1">
        {[...new Set(products.map((p) => p.brand).filter(Boolean))].slice(0, 8).map((b) => (
          <button key={b} onClick={() => setBrandFilter(brandFilter === b ? '' : b!)}
            className={`text-[10px] px-2 py-1 rounded-full border transition-all ${brandFilter === b ? 'bg-gold/10 border-gold text-foreground' : 'border-border text-muted-foreground hover:border-gold/50'}`}
          >{b}</button>
        ))}
      </div>
              ...
              <div><Label className="text-xs">Inventory Type</Label>
                <div className="flex gap-2 mt-1">
                  {(['backbar', 'retail'] as InventoryType[]).map((t) => (
                    <button key={t} onClick={() => setFormType(t)} className={`flex-1 py-2 rounded-lg border text-xs font-medium ${formType === t ? 'border-gold bg-gold/10' : 'border-border'}`}>
