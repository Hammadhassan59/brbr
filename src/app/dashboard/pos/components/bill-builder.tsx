...
export function BillBuilder({
  ...
  return (
    <div className="flex h-full gap-0 relative">
      <div className="flex-1 flex flex-col min-w-0 bg-background lg:border-r border-border">
        ...
        <div className="flex-1 overflow-y-auto px-3 pb-2" style={{ scrollbarWidth: 'none' }}>
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-3 xl:grid-cols-4 gap-2 pb-16 lg:pb-0">
      ...
      {/* Mobile bottom bar: tap to expand bill */}
      <div className="lg:hidden fixed bottom-0 left-0 right-0 z-30 bg-card border-t border-border">
        {mobileShowBill ? (
          <div className="max-h-[70vh] flex flex-col">
            ...
            <div className="flex-1 overflow-y-auto px-3 py-2 space-y-2 max-h-[40vh]" style={{ scrollbarWidth: 'none' }}>
              {items.length === 0 ? (
                <p className="text-xs text-muted-foreground text-center py-4">No items yet</p>
              ) : (
                items.map((item) => (
                  <div key={item.id} className="bg-background border border-border rounded-lg p-3 flex items-center gap-2 text-sm">
                    <div className="flex-1 min-w-0">
                      <p className="font-medium truncate">{item.name}</p>
                      {item.stylistName && <p className="text-[10px] text-muted-foreground">{item.stylistName}</p>}
                    </div>
                    <span className="text-xs font-medium">{formatPKR(item.totalPrice)}</span>
          ...
          <button onClick={() => setMobileShowBill(true)} className="w-full flex items-center justify-between px-4 py-3">
      ...
      {/* Desktop bill panel */}
      <div className="hidden lg:flex w-[320px] lg:w-[340px] flex-col bg-card shrink-0">
