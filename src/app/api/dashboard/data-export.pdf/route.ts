...
export async function GET() {
  ...
  // ─── Inventory snapshot ───
  // Post-037 each product lives in exactly one branch; current_stock lives
  ...
  if (productIds.length > 0) {
    ...
    for (const r of (bp || []) as Array<{ product_id: string; current_stock: number }>) {
      branchProductStock.set(r.product_id, Number(r.current_stock) || 0);
    }
  }
  const productsWithStock = (products || []).map((p: { id: string; branch_id?: string | null }) => ({
    ...p,
    current_stock: branchProductStock.get(p.id) ?? 0,
    branch_name: p.branch_id ? branchById.get(p.branch_id)?.name ?? '—' : '—',
  }));
  pdf.section(`Inventory (${productsWithStock.length} active products)`);
  pdf.table(productsWithStock, [
    { key: 'name', label: 'Product', width: 0.28 },
    { key: 'brand', label: 'Brand', width: 0.14 },
    { key: 'branch_name', label: 'Branch', width: 0.14 },
    { key: 'current_stock', label: 'Stock', width: 0.12, align: 'right' },
    { key: 'unit', label: 'Unit', width: 0.08 },
    { key: 'purchase_price', label: 'Cost', width: 0.12, align: 'right', format: (v) => fmtPKR(Number(v)) },
    { key: 'retail_price', label: 'Retail', width: 0.12, align: 'right', format: (v) => fmtPKR(Number(v)) },
  ]);
